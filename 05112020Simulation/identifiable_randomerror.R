...
lambda.seq <- exp(seq(3, 3, 1))
rank.est1 <- rep(0, length(lambda.seq))
converge.iter1 <- rep(0, length(lambda.seq))
num_obj <- rep(0, length(lambda.seq))
...
for (i in 1: length(lambda.seq)) {
  ...
  rank.est1[i] <- B_iter_next_rank
  converge.iter1[i] <- iter.count
