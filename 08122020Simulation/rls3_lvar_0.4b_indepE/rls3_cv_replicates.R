...
evaluation <- function(k){ 
  ...
  # range of lambda
  repl <- 50
  lambda.seq <- exp(c(2.2, 2.25, 2.3, 2.35, 2.4, 2.45, 2.5, 2.6, 2.65, 2.7, 2.8, 2.9, 3.1, 3.3, 3.5, 3.8, 4.8))
  ...
  estb <- matrix(0, nrow = repl, ncol = k)
  ...
  for (p in 1:repl) {
    ...
    while ((((disB > vareps) || (disT > vareps) || (disbeta > vareps))  && ((disXB > vareps) || (disT > vareps) || (disbeta > vareps))) && (iter.count < 3000)) {
      ...
      B_iter_next <- rrr(X2, Z_iter_B, lambda = tune_lam)$CShat
      B_iter_next_rank <- rrr(X2, Z_iter_B, lambda = tune_lam)$r
      ...
      iter_weight <- sort(rrr(X2, Z_iter_B, lambda = tune_lam)$weight)
      disT <- sum((T_iter - T_iter_next)^2)
      disB <- sum((B_iter - B_iter_next)^2)
      disbeta <- sum((b_iter - b_iter_next)^2)
    ...
    num_obj[p] <- 1/2 * 1/2 * sum((Z - X2 %*% B_iter - matrix(rep(T_iter, each = k), nrow = n, byrow = T) -  matrix(rep(b_iter, times = n), nrow = n, byrow = T))^2) + 
      lambda.seq[p] * sum(iter_weight*svd(X2 %*% B_iter)$d)
    Z.est <- X2 %*% B_iter + matrix(rep(mean(T_iter), n*k), nrow = n, byrow = T) +  matrix(rep(b_iter, times = n), nrow = n, byrow = T)
    # err[p] <-  sum((Zv - Z.est)^2)
    # fit_err[l] <- sum((Z - Z.est)^2)
    # fit_B[l] <- sum((B2-B_iter[-1, ])^2)
    # fit_XB[l] <- sum((X2 %*% B2 - X2 %*% B_iter[-1, ])^2)
    esterr_B[p] <- sum((B2-B_iter)^2)
    esterr_XB[p] <- sum((X2 %*% B2 - X2 %*% B_iter)^2)
    estT[p,] <- T_iter
    estb[p,] <- b_iter
  ...
  write.csv(estb, file = 'estb.csv', row.names = FALSE)
  end_time <- Sys.time()
