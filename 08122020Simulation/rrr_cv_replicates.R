  ...
  for (p in 1:repl) {
    ...
    for (j in 1:10) {
      ...
      for (i in 1: length(lambda.seq)) {
        B_iter <- rrr(X_t, Z_t, lambda = lambda.seq[i])$CShat
        B_iter_rank <- rrr(X_t, Z_t, lambda = lambda.seq[i])$r
        Z.est <-  X_v %*% B_iter
    ...
    B_iter <- rrr(X, Z, lambda = tune_lam)$CShat
    B_iter_rank <- rrr(X, Z, lambda = tune_lam)$r
      
    iter_weight <- sort(rrr(X, Z, lambda = tune_lam)$weight)
    
    rank.est[p] <- B_iter_rank
    num_obj[p] <- 1/2 * sum((Z - X %*% B_iter)^2) + tune_lam * sum(iter_weight*svd(X %*% B_iter)$d)
    Z.est <-  X %*% B_iter
