...
X <-matrix(rep(1, m*n), nrow = m, ncol = n) + X.raw %*% diag(sqrt(pmax(Sigma.eigen$values, 0))) %*%
                      t(Sigma.eigen$vectors)
...
# X cannot be centered since it has to be a invertible matrix 
# rho <- 0.4
# Sigma <- diag(1, n)
# for (i in 1:(n-1)){
#   for (j in (i+1):n){
...
X.mean <-  matrix(rep(rowMeans(X), each = n), ncol = n, byrow = T)
X <- X - X.mean
...
for (i in 1: length(lambda.seq)) {
  ...
  png(paste0("plot-B-algcent", i, ".png"))
  plot(B, B_iter)
  dev.off()

  # save image for T matrix
  png(paste0("plot-T-algcent", i, ".png"))
  plot(T, T_iter)
  dev.off()

  # save image for XB matrix
  png(paste0("plot-XB-algcent", i, ".png"))
  plot(B %*% X, t(B_iter) %*% X)
  dev.off()

  # save image for Z matrix
  png(paste0("plot-Z-algcent", i, ".png"))
  plot(Z, Z.est)
  dev.off()
}
