...
for (j in 1:10) {
  ...
  for (i in 1: length(lambda.seq)) {
    B_est <- rrr(X_t, Z_t, lambda = lambda.seq[i])$CShat
    B_est_rank <- rrr(X_t, Z_t, lambda = lambda.seq[i])$r
    iter_weight <- sort(rrr(X_t, Z_t, lambda = lambda.seq[i])$weight)
...
for (l in 1: length(lambda.seq)){
  B_est <- rrr(X, Z, lambda = lambda.seq[l])$CShat
  B_est_rank <- rrr(X, Z, lambda = lambda.seq[l])$r
  iter_weight <- sort(rrr(X, Z, lambda = lambda.seq[l])$weight)
  
  rank.est[l] <- B_est_rank
  num_obj[l] <- 1/2 * sum((Z - X %*% B_est)^2) + lambda.seq[l] * sum(iter_weight*svd(X %*% B_est)$d)
  Z.est <-  X %*% B_est
  err[l] <-  sum((Z_v - Z.est)^2)
  fit_err[l] <- sum((Z - Z.est)^2)
  fit_B[l] <- sum((B2-B_est[-1, ])^2)
  fit_XB[l] <- sum((X2 %*% B2 - X2 %*% B_est[-1, ])^2)
}
ed_time <- Sys.time()
...
# png(paste0("plot-B-rrr-wocT.png"))
# plot(B2, B_est[-1, ])
# abline(a = 0, b = 1)
# dev.off()
# 
# # save image for XB matrix
# png(paste0("plot-XB-rrr-wocT.png"))
...
# 
# # save image for T matrix
# png(paste0("plot-beta-rrr-wocT.png"))
# plot(b, B_est[1,])
# abline(a = 0, b = 1)
# dev.off()
# 
# # save image for Z matrix
# png(paste0("plot-Z-rrr-wocT.png"))
# plot(Z, Z.est)
# abline(a = 0, b = 1)
# dev.off()
