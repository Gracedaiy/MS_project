...
for (j in 1:10) {
  ...
  for (i in 1: length(lambda.seq)) {
    ...
    write.csv(B_iter, file = paste0('B', j, '.csv'), row.names = FALSE)
  }
}
end_time <- Sys.time()
...
# png(paste0("plot-B-ls2-wocT.png"))
# plot(B2, B_iter[-1, ])
# abline(a = 0, b = 1)
# dev.off()
# 
# # save image for XB matrix
# png(paste0("plot-XB-ls2-wocT.png"))
# plot(X2 %*% B2,  X2 %*% B_iter[-1,])
# abline(a = 0, b = 1)
# dev.off()
# 
# # save image for T matrix
# png(paste0("plot-T-ls2-wocT.png"))
# plot(T, T_iter)
# abline(a = 0, b = 1)
# dev.off()
# 
# # save image for T matrix
# png(paste0("plot-beta-ls2-wocT.png"))
# plot(b, B_iter[1,])
# abline(a = 0, b = 1)
# dev.off()
# 
# # save image for Z matrix
# png(paste0("plot-Z-ls2-wocT.png"))
# plot(Z, Z.est)
# abline(a = 0, b = 1)
# dev.off()
