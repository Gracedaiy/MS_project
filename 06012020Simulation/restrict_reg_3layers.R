...
for (i in 1: length(lambda.seq)) {
  ...
  # y <- as.vector(Z - t(B_iter) %*% X - b %*% matrix(rep(1,n), nrow = 1))
  ...
  while ((((disB > vareps) || (disT > vareps))  && ((disXB > vareps) || (disT > vareps))) && (iter.count < 3000)) {
    ...
    y_iter <- as.vector(Z - t(B_iter) %*% X - b_iter %*% matrix(rep(1,n), nrow = 1))
  ...
  png(paste0("plot-B-rlsc-l3.png"))
  plot(B, t(B_iter))
  dev.off()

  # save image for XB matrix
  png(paste0("plot-XB-rlsc-l3.png"))
  plot(B %*% X, t(B_iter) %*% X)
  abline(a = 0, b = 1)
  dev.off()
  
  # save image for T matrix
  png(paste0("plot-T-rlsc-l3.png"))
  plot(T, T_iter)
  abline(a = 0, b = 1)
  dev.off()
  
  # save image for T matrix
  png(paste0("plot-beta-rlsc-l3.png"))
  plot(b, b_iter)
  abline(a = 0, b = 1)
  dev.off()
  
  # save image for Z matrix
  png(paste0("plot-Z-rlsc-l3.png"))
  plot(Z, Z.est)
  abline(a = 0, b = 1)
  dev.off()
}
