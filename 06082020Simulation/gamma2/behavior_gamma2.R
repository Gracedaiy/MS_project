...
X <- matrix(rnorm(m*n), nrow = m, ncol = n, byrow = TRUE)
...
Z <- B %*% X + b %*% matrix(rep(1,n), nrow = 1)+  matrix(rep(1, k), ncol = 1) %*% T + E

# convergence criterion
vareps <- 10^(-3)

# range of lambda
lambda.seq <- exp(seq(13, 14, 1))
...
for (i in 1: length(lambda.seq)) {
  ...
  # initial estimated b, reset the seed
  set.seed(Sys.time())
  ...
  while ((((disB > vareps) || (disT > vareps) || (disbeta > vareps))  && ((disXB > vareps) || (disT > vareps) || (disbeta > vareps))) && (iter.count < 3000)) {
    ...
    Z_iter_b <- t(Z) - t(X) %*% B_iter - T_iter %*% t(matrix(rep(1, k), ncol = 1))
  ...
  num_obj[i] <- 1/2 * sum((t(Z) - t(X) %*% B_iter - T_iter %*% matrix(rep(1, k), nrow = 1) - t(b_iter %*% matrix(rep(1,n), nrow = 1)))^2) + 
    lambda.seq[i] * sum(iter_weight*svd(t(X) %*% B_iter)$d)
  Z.est <-  t(B_iter) %*% X + matrix(rep(1, k), ncol = 1) %*% t(T_iter)
  
  # # save image for B matrix
  # png(paste0("plot-B-ls-3steps-gamma2.png"))
  # plot(B, t(B_iter))
  # abline(a = 0, b = 1)
  # dev.off()
  # 
  # # save image for XB matrix
  # png(paste0("plot-XB-ls-3setps-gamma2.png"))
  # plot(B %*% X, t(B_iter) %*% X)
  # abline(a = 0, b = 1)
  # dev.off()
  # 
  # # save image for T matrix
  # png(paste0("plot-T-ls-3steps-gamma2.png"))
  # plot(T, T_iter)
  # abline(a = 0, b = 1)
  # dev.off()
  # 
  # # save image for T matrix
  # png(paste0("plot-beta-ls-3steps-gamma2.png"))
  # plot(b, b_iter)
  # abline(a = 0, b = 1)
  # dev.off()
  # 
  # # save image for Z matrix
  # png(paste0("plot-Z-ls-3steps-gamma2.png"))
  # plot(Z, Z.est)
  # abline(a = 0, b = 1)
  # dev.off()
}
...
for (i in 1: length(lambda.seq)) {
  ...
  converge.iter.rls[i] <- iter.count
  num_obj.rls[i] <- 1/2 * sum((t(Z) - t(X) %*% B_iter - T_iter %*% matrix(rep(1, k), nrow = 1) - t(b_iter %*% matrix(rep(1,n), nrow = 1)))^2) + 
    lambda.seq[i] * sum(iter_weight*svd(t(X) %*% B_iter)$d)
  Z.est <-  t(B_iter) %*% X + matrix(rep(1, k), ncol = 1) %*% t(T_iter)

  
  # save image for B matrix
  png(paste0("plot-B-rls-3steps-gamma2.png"))
  plot(B, t(B_iter))
  abline(a = 0, b = 1)
  dev.off()

  # save image for XB matrix
  png(paste0("plot-XB-rls-3setps-gamma2.png"))
  plot(B %*% X, t(B_iter) %*% X)
  abline(a = 0, b = 1)
  dev.off()

  # save image for T matrix
  png(paste0("plot-T-rls-3steps-gamma2.png"))
  plot(T, T_iter)
  abline(a = 0, b = 1)
  dev.off()

  # save image for T matrix
  png(paste0("plot-beta-rls-3steps-gamma2.png"))
  plot(b, b_iter)
  abline(a = 0, b = 1)
  dev.off()

  # save image for Z matrix
  png(paste0("plot-Z-rls-3steps-gamma2.png"))
  plot(Z, Z.est)
  abline(a = 0, b = 1)
  dev.off()
}
