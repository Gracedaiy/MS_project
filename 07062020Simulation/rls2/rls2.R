...
for (i in 1: length(lambda.seq)) {
  ...
  png(paste0("plot-B-rls2-wocT-op.png"))
  plot(B2, B_iter[-1, ])
  abline(a = 0, b = 1)
  dev.off()

  # save image for XB matrix
  png(paste0("plot-XB-rls2-wocT-op.png"))
  plot(X2 %*% B2,  X2 %*% B_iter[-1,])
  abline(a = 0, b = 1)
  dev.off()

  # save image for T matrix
  png(paste0("plot-T-rls2-wocT-op.png"))
  plot(T, T_iter)
  abline(a = 0, b = 1)
  dev.off()

  # save image for T matrix
  png(paste0("plot-beta-rls2-wocT-op.png"))
  plot(b, B_iter[1,])
  abline(a = 0, b = 1)
  dev.off()

  # save image for Z matrix
  png(paste0("plot-Z-rls2-wocT-op.png"))
  plot(Z, Z.est)
  abline(a = 0, b = 1)
  dev.off()
}
