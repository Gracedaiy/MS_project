...
evaluation <- function(k){ 
  ...
  for (l in 1: length(lambda.seq)){
    ...
    T_iter <- matrix(rowMeans(Zt - Xt %*% B_iter), ncol = 1)
    ...
    num_obj[l] <- 1/2 * sum((Zt - Xt %*% B_iter - matrix(rep(T_iter, each = k), nrow = n-10, byrow = T))^2) + 
