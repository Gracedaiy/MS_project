  ...
  for (p in 1:repl) {
    ...
    for (j in 1:(n/10)) {
      ...
      for (i in 1: length(lambda.seq)) {
        ...
        Z.est.newval <-  X2 %*% B_iter + matrix(mean(T_iter), nrow = n, ncol = k ) + matrix(rep(b_iter, times = n), nrow = n, byrow = T)
    ...
    for (l in 1: length(lambda.seq)){
      ...
      # initial estimated B
      B_iter <- ginv(t(Xt)%*%Xt)%*%t(Xt)%*% Zt
      
      # initial estimated b
      b_iter <- b_start
      
      # initial estimated T (RLS)
      T_iter <- matrix(rowMeans(Zt - Xt %*% B_iter - matrix(rep(b_iter, times = n-10), nrow = n-10, byrow = T)), ncol = 1)
      T_iter <- T_iter - mean(T_iter)
      
      
      while ((((disB > vareps) || (disT > vareps) || (disbeta > vareps))  && ((disXB > vareps) || (disT > vareps) || (disbeta > vareps))) && (iter.count < 3000)) {
        Z_iter_B <- Zt - matrix(rep(T_iter, each = k), nrow = n-10, byrow = T) - matrix(rep(b_iter, times = n-10), nrow = n-10, byrow = T)
        
        B_iter_next <- rrr(Xt, Z_iter_B, lambda = lambda.seq[l])$CShat
        B_iter_next_rank <- rrr(Xt, Z_iter_B, lambda = lambda.seq[l])$r
        
        Z_iter_b <- Zt - Xt %*% B_iter - matrix(rep(T_iter, each = k), nrow = n-10, byrow = T)
        b_iter_next <- matrix(colMeans(Z_iter_b), nrow = 1)
        
        # Using RLS method
        T_iter_next <- matrix(rowMeans(Zt - Xt %*% B_iter - matrix(rep(b_iter, times = n-10), nrow = n-10, byrow = T)), ncol = 1)
        T_iter_next <- T_iter_next - mean(T_iter_next)
        
        iter_weight <- sort(rrr(Xt, Z_iter_B, lambda = lambda.seq[l])$weight)
        disT <- sum((T_iter - T_iter_next)^2)
        disB <- sum((B_iter - B_iter_next)^2)
        disbeta <- sum((b_iter - b_iter_next)^2)
        disXB <- sum((Xt %*% B_iter - Xt %*% B_iter_next)^2)
        # print(c(disT, disB, disXB))
        # print(sum(T_iter))
      ...
      num_obj[l] <- 1/2 * sum((Zt - Xt %*% B_iter - matrix(rep(T_iter, each = k), nrow = n-10, byrow = T) -  matrix(rep(b_iter, times = n-10), nrow = n-10, byrow = T))^2) + 
        lambda.seq[l] * sum(iter_weight*svd(Xt %*% B_iter)$d)
